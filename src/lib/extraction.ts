...
export function buildExtractionPrompt(catalog: CatalogEntry[]): string {
  ...
  return [
    ...
    "If a printed result does not match any catalog marker, add its printed name to",
    "`unmatched` (do not guess a key). If the same marker appears more than once,",
    "use the most recent / primary result.",
    "",
